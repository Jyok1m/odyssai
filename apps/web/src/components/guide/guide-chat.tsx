...
export function GuideChat() {
  ...
  return (
    <Container as="section" className="pb-24 sm:pb-32">
      {/* Le seul objet interactif du site : un panneau borde le detache de la
          prose, et il tombe juste sous le hero pour se voir sans defiler. */}
      <div className="rounded-card border border-line bg-abyss px-6 py-8 sm:px-10 sm:py-10">
        <div className="mx-auto max-w-headline">
          <h2 className="font-voice text-title text-balance text-vellum">
            {t("title")}
          </h2>
          <p className="mt-4 text-ui text-pretty text-vellum-2">{t("intro")}</p>
            ...
            <ol className="mt-8 flex flex-col gap-6">
              {turns.map((turn, index) => (
                <li key={index} className="flex flex-col gap-3">
                  ...
                  {/* Plus sombre que le panneau, quand la bulle du joueur est
                      plus claire : les deux voix se distinguent sans couleur. */}
                  <div
                    className="rounded-card border border-line bg-ink px-4 py-3"
                  ...
                  >
                    ...
                    {turn.answer ? (
                      ...
                      !turn.error && (
                        <p className="mt-1.5 text-ui text-vellum-3">
                          {t("thinking")}
                        </p>
                      )
                    )}
          ...
          {/* Annoncee d'un bloc a la fin : un lecteur d'ecran noierait
          ...
          >
            <div className="rounded-card border border-line bg-ink px-3.5 pt-3 pb-2.5 transition-colors focus-within:border-accent">
              <label
                htmlFor="guide-question"
                className="block text-caption text-vellum-3"
              >
                {t("inputLabel")}
              </label>
              <textarea
                ...
                onKeyDown={(event) => {
                  // Pendant une composition IME, Entree valide un caractere et
