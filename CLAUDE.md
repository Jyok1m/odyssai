...
- **La route est gardée par la phase de l'alpha**, celle de `site_settings` que le tableau de bord règle, et non plus par un drapeau de compilation : ouvrir le jeu est une décision qui se prend un matin, pas un déploiement. Côté api, `AlphaOpenGuard` après `SessionGuard` sur toutes les routes de jeu, 403 `alpha_closed` ; c'est lui la règle. Côté web, une commodité : `GameLink` répond par un toast au lieu de naviguer, `GameGate` affiche une page qui le dit au lieu d'un 404, la phase étant publique de toute façon. Un administrateur passe toujours, des deux côtés : c'est ainsi qu'on vérifie la production avant d'ouvrir. `NEXT_PUBLIC_ALPHA_OPEN` n'existe plus, ni dans le Jenkinsfile ni dans l'image.
...
- **Le récit se lit en PDF**, dans un onglet : `story-pdf.tsx`, rendu par `@react-pdf/renderer` dans le navigateur, importé au clic pour ne pas peser sur la table. Les scènes du meneur seulement, sans les phrases du joueur ni les réponses à ses questions, avec en tête le titre, le monde, le personnage, l'acte et la date. Polices de base du PDF, qui couvrent le français sans rien charger. L'onglet s'ouvre dans le clic, avant le rendu : ouvert après, le navigateur le prendrait pour une fenêtre surgissante. Aucune vue de lecture dans la page : c'était le même texte sous une autre taille, et le PDF est ce qu'on relit.
- **Une question au meneur et sa réponse se lisent en aparté** : `request` est porté par le message du joueur et par la réponse (`TurnMessage`), et le fil les groupe dans un bloc à part, teinte arcane. Le meneur y parle en `font-voice` comme ailleurs, mais ce n'est pas une scène.
...
- **Chaque prompt dit les bornes que son schéma tient, en mots et sur la clé elle-même** (`abstraction/v5`, `generation/v8`), depuis `THEME_LIMITS` et `GENERATION_LIMITS` dans `schemas/world.ts`, lus par les schémas Zod et convertis par `wordsWithin` (huit caractères par mot, marge comprise). Les recopier en clair ferait deux vérités, et la fausse serait celle que le modèle lit. Un modèle ne respecte pas une borne qu'il ignore : la génération échouait sur un `setting` de plus de 400 caractères, puis la charte sur un `allowed` de plus de 200, la doctrine lui demandant d'écrire chaque chose « avec sa règle » sans dire la place qu'elle avait pour cela. **Dite en caractères, une borne lui sert de cible** : mesuré sur `qwen3.7-plus`, geography 1335 pour 1200, era 379 pour 300, et à ce volume il oubliait `accentHue`. En mots, avec la marge, le nœud `lore` passe trois fois sur trois et une génération entière tient en sept appels sans rejeu. Contrepartie : il n'emplit qu'à moitié le budget de mots (geography autour de 500 caractères), et `CHARS_PER_WORD` est le curseur si le lore paraît court.
...
- Le modèle de narration **se choisit par évaluation**, pas par réputation. `LLM_NARRATOR_CANDIDATES` porte les modèles à comparer, `pnpm --filter @odyssai/api eval:narration` les fait tourner sur `packages/narrator/evals/abstraction.{fr,en}.jsonl`, et le gagnant se reporte dans `models.ts`. Mesuré à outil égal : `qwen3.5-27b` 92 % fr / 100 % en, `qwen3.5-35b-a3b` 92 % / 92 %, les échecs étant tous des JSON incomplets, jamais un nom emprunté. Remesuré avec `abstraction/v4` le 23 septembre 2026 : `qwen3.5-9b` 17 % fr / 33 % en, `qwen3.5-35b-a3b` 58 % / 92 %, `qwen3.7-plus` 83 % / 100 %, `qwen3.5-27b` 92 % / 92 %. Les échecs sont presque tous un `setting` au-delà de ses 400 caractères en français : **un petit modèle ne compte pas les caractères**, même quand le prompt les lui dit, et c'est ce qui faisait échouer toute génération quand l'abstraction tournait sur le 9b. Le pire côté décide, donc le 27b. En `abstraction/v5` (longueurs en mots), le 27b fait 83 % fr / 100 % en, ses deux échecs français étant la garde sur les noms propres et non plus une longueur ; à douze cas par langue, un cas vaut huit points.
...
- **Un préfixe BullMQ par environnement** (`BULLMQ_PREFIX`, `bull` par défaut, `QueuePrefixSchema` partagé par l'api qui publie et le worker qui consomme). Le poste de développement partage le Redis du serveur par le tunnel, et le `worker-dev` qui y tourne prenait les travaux publiés par une api locale : on testait le code du serveur en croyant tester le sien, et une journée d'échecs « malgré les correctifs » venait de là. `bull-local` dans le `.env` du poste, rien à changer côté ansible.
...
- **Chaque Dockerfile recopie ses dépendances internes à la main**, manifeste puis sources, parce que l'install y est filtrée et que turbo n'y est pas. Ajouter un `@odyssai/*` au `package.json` d'une app sans toucher à son Dockerfile passe donc tout `make check` et toutes les suites (le workspace entier est présent en local) et ne tombe qu'au `docker build` de la CI : c'est arrivé au worker avec `engine`. Le contrôle qui manque est `docker build -f apps/<app>/Dockerfile .` avant de committer une dépendance interne nouvelle.
...
### Des mondes qui ne se ressemblent pas

Les mêmes prénoms revenaient d'un monde à l'autre et chaque histoire était un complot d'organisation de l'ombre. Mesuré sur six inspirations, charte, factions et personnages, avec `generation/v8` et `qwen3.7-plus` : 25 PNJ, **14 prénoms distincts**, Kaelen ×4, Elara ×4, Mira ×4, et « la maison Varek » recopiée du prompt. Deux causes, aucune n'est le hasard.

- **Le modèle recopie les exemples du prompt.** Le meneur donnait « Kaelen, qui t'a formé » en exemple de présentation ; la génération donnait « la maison Varek ». Les exemples nommés sont remplacés par « Untel », et la règle commune dit que les exemples restants ne se reprennent pas. `Varek` est dans la liste des interdits pour la même raison.
- **Laissé libre, il retombe sur ses pentes.** Le code tire donc avant chaque génération un **registre** d'histoire (`STORY_REGISTERS` : enquête, dette, fête, chantier, héritage, route, procès, disparition, commerce, rivalité, saison, voyage, concours, malentendu) et une **palette** de noms (`NAME_PALETTES` : bref, latin, nordique, méridional, slave, ouvert, composé, ancien), `drawFlavour` dans `engine`, gardés dans l'état du graphe pour qu'une reprise ne retire pas. Le prompt les porte en tête de la saisie (`<registre>`, `<noms>`) et dit qu'ils s'imposent à la charte comme à l'arc. Les palettes sont **décrites, jamais illustrées** : un exemple serait recopié.
- **Le complot est interdit par défaut**, avec la prophétie, l'élu et le mal ancien : c'est la pente de tout modèle, et elle ne s'ouvre que si le registre tiré l'appelle en toutes lettres.
- **Les prénoms trop vus sont interdits et vérifiés** : `OVERUSED_NAMES` et `OVERUSED_GROUP_WORDS` dans `schemas/world.ts`, dits aux prompts de génération et du meneur, et relus par le graphe sur les factions et les personnages (`overusedNamesIn`, mots entiers repliés) : un nœud qui en rend un est rejoué comme une forme invalide. Pas dans `WorldBibleSchema`, qui est relu à chaque tour : un monde d'avant qui porte un Kaelen doit rester jouable.
- **Le modèle de génération se choisit sur cette mesure.** Mêmes six inspirations, mêmes tirages, `generation/v9` : `qwen3.7-plus` rend 21 prénoms distincts sur 24 et glisse encore trois interdits ; `deepseek-v4-flash` en rend 33 sur 35, aucun interdit, et suit les palettes à la lettre ; `skyfall-36b-v2` est bridé par le fournisseur (429 au quatrième monde). La génération passe donc sur `deepseek-v4-flash` ; le lore de partie reste sur `qwen3.7-plus`, trois phrases sans nom nouveau, la question ne s'y pose pas. Le meneur (`qwen3.8-27b`) garde son modèle : il reçoit la liste, et rien n'a été mesuré sur lui.
...
- **C'est le code qui dit si le dé a tranché, pas le modèle.** Il déclarait `usedDie` en queue de réponse et se trompait dans les deux sens : faux là où le joueur venait de lancer, vrai sur une simple question, ce qui affichait « défavorable » sous une question posée au batelier. Le verdict montré dans la bulle et `turns.used_die` viennent de `settled` seul, la décision prise avant l'appel, et le bloc de queue ne porte plus la clé (v23). Sur un tour qu'on lui laisse juger, la bande peut colorer son récit, mais rien ne s'affiche : le joueur n'a rien lancé.
...
Le meneur écrivait les répliques lui-même, dans sa propre voix, et tous les personnages finissaient par parler comme lui. Un second appel, à part, joue le personnage à qui le joueur s'adresse avec sa seule carte sous les yeux (rôle `dialogue` dans `models.ts`). Il a d'abord tourné sur `gryphe/mythomax-l2-13b`, un finetune de jeu de rôle censé tenir une voix, refuser, mentir et ne pas lisser. Relevé sur la première partie en production (`turns.line`) : une réplique sur trois, les deux autres étant la traduction du message du joueur. Le rôle tourne depuis sur le modèle du tour, et c'est l'appel séparé, avec la carte seule, qui tient la voix.
...
- **Il joue avant le meneur, et le meneur porte sa phrase telle quelle** (`turn/v23`, bloc `<replique>`). Il voit la carte du personnage, caché compris, le ton de la charte, les deux derniers tours et le message du joueur. Il ne raconte rien, ne décrit rien, ne parle pas pour le joueur : c'est le meneur qui tient la scène, lui ne tient qu'une voix.
- **Ce qu'un modèle de jeu de rôle rend malgré la consigne se nettoie** (`cleanLine`) : le nom en tête, les guillemets, les didascalies entre astérisques, le récit posé autour des guillemets (ce qui est cité est la réplique, le reste n'est pas à lui), la tirade coupée à la fin d'une phrase. Relu par la couche lexicale et par la garde sur les emprunts, comme la réponse du meneur : une réplique refusée est une réplique absente, jamais un tour perdu. Le meneur la fait alors parler lui-même, comme avant. **Une ligne qui répète le message du joueur est écartée** (`echoes`) : le meneur l'ignorait de lui-même, mais `turns.line` gardait une ligne que le récit n'avait jamais portée, et la table ne disait plus si le modèle de dialogue servait. Repli des titres d'œuvres, mots de trois lettres au moins, quatre mots pleins ou l'égalité : une réponse qui reprend des mots du joueur pour y ajouter les siens passe, c'est la part de ses mots qui compte.
- **Fondu dans le tour** (`CREDIT_COSTS.dialogue` à zéro) : trois phrases sur un prompt court, une fraction du meneur. Le curseur existe pour le jour où ce ne serait plus vrai. `llm_usage` le journalise sous `dialogue`, et `turns.speaker` et `turns.line` gardent qui a parlé et ce qu'il a dit, pour relire ce que le meneur a reçu et ce qu'il en a fait.
- **La réplique se joue dans la langue du joueur** (`dialogue/v3`), et son message arrive seul, sans consigne collée derrière. La v2 faisait jouer en anglais, MythoMax ayant un français faible, et rappelait la langue en queue de prompt, juste après le message en français : la mesure qui l'a validée (5 sur 5) comptait la langue de la sortie, pas si c'était une réponse. Un texte suivi de « answer in English, the line alone » a la silhouette d'une tâche de traduction, et un petit modèle la fait. La langue et l'interdiction de répéter vivent dans le système ; la consigne du meneur garde le cas d'une ligne arrivée en anglais, qu'il traduit, et c'est la seule liberté qu'il prend. `turns.line` garde ce que le meneur a reçu.
...
Le jeu est en ligne pour tout le monde. Il n'y a plus de porte à cent places : les cent premiers inscrits reçoivent leur bonus (`FOUNDER_BONUS`, trente crédits, rang lu sur `created_at`, administrateurs non comptés), le cent unième entre quand même, sans le bonus. `ALPHA_SEATS`, `AlphaFullError`, `alpha_full` et le refus avant le realm n'existent plus.

- **Deux interrupteurs au tableau de bord**, dans `site_settings`. La phase (`open`, ou `preregistration` qui vaut désormais « fermé ») ouvre le jeu : `AlphaOpenGuard` côté api, `GameGate` et `GameLink` côté web, un administrateur passe toujours. On ne ferme plus que pour une maintenance ou une remise à zéro, et les textes le disent ainsi. La migration `alpha_live` a ouvert la phase au déploiement et mis le défaut à `open`.
- **La vente des paliers est fermée le temps de l'alpha** (`sales_open`, faux par défaut). `GET /billing/catalog` porte `salesOpen`, `purchasable` le respecte, `POST /billing/checkout` répond 403 `sales_closed`, et la page des tarifs l'explique au lieu de cacher les paliers : ils sont ce vers quoi on va. Elle s'ouvre au tableau de bord, un matin.
- **Ce que le site dit de l'alpha, partout pareil** : jouable maintenant, des bugs possibles à signaler depuis le bouton dédié, trente crédits de plus pour les cent premiers, pas d'abonnement, une progression qui peut être remise à zéro et des crédits qui restent. Le bandeau (`AlphaNotice`), le hero (`AlphaStanding`, qui distingue qui a un compte de qui n'en a pas), la page des tarifs, les conditions générales (section « Accès pendant l'alpha »), la FAQ et le prompt du guide (`guide/v5`) portent la même chose. Un texte qui dirait encore « pré-inscription » ou « cent places » est à corriger.
- `GET /alpha` reste public et sert `founders` (places, crédits, prises, restantes) et `salesOpen` avec la phase : le bandeau doit s'afficher avant que qui que ce soit se connecte.
- **Un joueur peut toujours créer une identité Keycloak sans passer par `/auth/signup`** ; il obtient maintenant sa ligne `users` à la première connexion, comme les autres. Le seul verrou qui resterait serait de couper l'auto-inscription du realm, dans le rôle ansible.
...
- Deux fontes, deux rôles : `font-voice` (Literata) pour le narrateur et les titres, `font-ui` (Instrument Sans) pour l'interface et le joueur. Le meneur parle en `font-voice` mais à la taille des autres conversations du site (`text-ui-sm`) : à 19 px, une colonne de jeu se lisait comme une page de roman. `text-narration` reste celle de la prose des pages de contenu.
- **Pas de phrase explicative dans les panneaux de jeu** (« tenu par le code », rappels de règles, sous-titres qui paraphrasent le titre) : le joueur joue, il ne lit pas la documentation. Un libellé de bouton dit ce que le bouton fait, et c'est tout ce qu'il y a à lire.
- `Definition` passe en deux colonnes par requête de conteneur (`@container` sur `Panel` et sur les cartes), jamais par le point de rupture de l'écran : dans une grille à trois cartes, un terme à largeur fixe étranglait la valeur.
