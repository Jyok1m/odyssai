...
- **La réplique se joue en anglais** (`dialogue/v2`), quelle que soit la langue du joueur, et c'est le meneur qui la rend en français : traduire, pas réécrire, même sens, même ton, même longueur, sa voix et non la sienne. MythoMax est un Llama 2 dont le français est moins sûr que son anglais, et le meneur relisait déjà la ligne pour en corriger la langue : autant lui donner la meilleure version à porter. Le prompt de dialogue n'a donc plus de locale, et `turns.line` garde la ligne anglaise, ce que le meneur a reçu. **La consigne de langue se répète en queue de prompt** : un finetune de treize milliards de paramètres suit le contexte plus que le système, et un contexte français le faisait répondre en français (mesuré : 0 sur 1 sans le rappel, 5 sur 5 avec). La consigne du meneur ne présume pas la langue de la ligne, pour le jour où il désobéirait quand même. Si la voix se perd à la traduction, c'est le modèle du rôle `dialogue` qu'on change, pas la mécanique.
