...
- **Ce qu'on annonce se règle au tableau de bord**, dans `site_settings`, une table à une seule ligne qu'une contrainte de vérification protège d'une seconde. Passer de la pré-inscription à l'ouverture est une décision qui se prend un matin, pas un déploiement. **La même phase ouvre le jeu** (voir « Parcours d'entrée en jeu ») : annoncer une alpha en cours dont les portes seraient fermées ferait mentir le site. `GET /alpha` la sert publiquement, comme le catalogue des paliers : le bandeau doit s'afficher avant que qui que ce soit se connecte.
