...
- Le récit ne relit pas les réponses aux questions du joueur : `request` est porté par la réponse du meneur dans `TurnMessage`, et `ask` n'est pas une scène. Il s'enregistre en PDF par l'impression du navigateur (`window.print`, `print:hidden` sur tout ce qui n'est pas lui, noir sur blanc dans `globals.css`) : pas de bibliothèque pour un document que le navigateur sait déjà produire.
...
- Deux fontes, deux rôles : `font-voice` (Literata) pour le narrateur et les titres, `font-ui` (Instrument Sans) pour l'interface et le joueur. `text-story` (17 px) est la taille de lecture de la table et du récit ; `text-narration` (19 px) reste celle de la prose des pages de contenu, trop grande pour une colonne de jeu.
