...
- **A debit is refunded once, and the database says so.** A refund carries the debit it cancels in `ref`, and the partial unique index `credit_entries_refund_ref_key` (`ref` where `reason = 'refund'`, raw SQL, Prisma cannot express it) refuses a second one. Several paths decide to refund by reading the ledger (a party member leaving, the settlement of a failed generation), and two concurrent readers both saw the debit as unreturned: credits were minted. The losing transaction rolls back its balance increment with it, and `refund()` logs it as already refunded. The migration fails if the ledger already holds two refunds of one debit: those are minted credits, to be read before deploying.
